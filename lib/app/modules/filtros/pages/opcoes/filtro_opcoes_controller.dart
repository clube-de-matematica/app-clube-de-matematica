...
class FiltroOpcoesController = _FiltroOpcoesControllerBase with _$FiltroOpcoesController;
...
abstract class _FiltroOpcoesControllerBase extends FiltroController with Store {
  ...
  final List<OpcaoFiltro> allOpcoes = List<OpcaoFiltro>();
  ...
  Iterable<Item> get _itensRelacionados {
    return filtrosTemp.allItens.where((item) {
      ...
      (
        ...
        filtrosTemp.niveis.any((element) => item.nivel == element.opcao)
      ) 
      &&
      (
        tipo == TiposFiltro.assunto ||
        filtrosTemp.assuntos.isEmpty ||
        filtrosTemp.assuntos.any((element) => item.assuntos.contains(element.opcao)) 
      )
      &&
      (
        tipo == TiposFiltro.dificuldade ||
        filtrosTemp.dificuldades.isEmpty ||
        filtrosTemp.dificuldades.any((element) => item.dificuldade == element.opcao)
      );
    });
  }
  ...
  Future<void> _setAllOpcoes() async {
      ...
      case TiposFiltro.assunto:
        opcoesRelacionadas = Assunto.instancias
          ...
          .where((a) => filtrosTemp.totalSelecinado == 0 
              ? true 
              : _itensRelacionados
                  .any((item) => item.assuntos
                      .any((b) => a == b || a.titulo == b.unidade)));
        break;
      case TiposFiltro.nivel:
        opcoesRelacionadas = Nivel.instancias
          .where((nivel) => filtrosTemp.totalSelecinado == 0 
              ? true 
              : _itensRelacionados.any((item) => item.nivel == nivel));
        break;
      case TiposFiltro.dificuldade:
        opcoesRelacionadas = Dificuldade.instancias
          .where((dificuldade) => filtrosTemp.totalSelecinado == 0 
              ? true 
              : _itensRelacionados.any((item) => item.dificuldade == dificuldade));
        break;
      case TiposFiltro.ano:
        opcoesRelacionadas = Ano.instancias
          .where((ano) => filtrosTemp.totalSelecinado == 0 
              ? true 
    ...
    if (tipo == TiposFiltro.assunto) {
      ...
      allOpcoes.addAll(allFilters[tipo].whereType<OpcaoFiltroAssuntoUnidade>());
      ...
      allOpcoes.addAll(
        opcoesRelacionadas.cast<Assunto>()
          ...
          .map<OpcaoFiltroAssuntoUnidade>(
            (unidade) => OpcaoFiltroAssuntoUnidade(unidade)
              ///Adicionar os assuntos que já estão no filtro.
              ..assuntos.addAll(
                allFilters[tipo].where((element) => unidade.titulo == 
                    (element.opcao as Assunto).unidade).cast<OpcaoFiltroAssunto>()
              )
          )
      );
      ...
      opcoesRelacionadas.cast<Assunto>()
        ...
        .forEach((assunto) {
          ...
          (allOpcoes.firstWhere((element) => 
              assunto.unidade == element.opcao.titulo) as OpcaoFiltroAssuntoUnidade).assuntos
            .add(OpcaoFiltroAssunto(assunto));
        });
      ///Ordenar os subassuntos.
      ///`removeDiacritics` é usado para desconsiderar os acentos.
      allOpcoes.forEach((e) {
        (e as OpcaoFiltroAssuntoUnidade).assuntos
            .sort((a, b) => removeDiacritics(a.opcao.toString())
      ...
      allOpcoes.addAll(
        opcoesRelacionadas.where((a) => !allFilters[tipo].any((b) => b.opcao == a))
    ...
    allOpcoes.sort((a, b) => removeDiacritics(a.opcao.toString())
        .compareTo(removeDiacritics(b.opcao.toString())));
    _loading.complete(await Future.delayed(const Duration(seconds: 1), () => false));
  }
  ...
  void onTapAssunto(OpcaoFiltroAssunto assunto, OpcaoFiltroAssuntoUnidade unidade) {
    onTap(assunto);
