...
abstract class _FiltroControllerBase with Store {
  ...
  bool get ativarAplicar {
    for (TiposFiltro key in allFilters.keys) {
      if (
        (allFilters[key].isNotEmpty || _filtrosAplicados.allFilters[key].isNotEmpty) &&
            allFilters[key].union(_filtrosAplicados.allFilters[key]).length != 
                allFilters[key].intersection(_filtrosAplicados.allFilters[key]).length
      ) return true;
    }
    return false;
  }
  ...
  String tiposFiltroToString(TiposFiltro tipoFiltro){
    switch (tipoFiltro){
      case TiposFiltro.assunto: return FILTRO_TEXTO_TIPO_ASSUNTO;
      case TiposFiltro.nivel: return FILTRO_TEXTO_TIPO_NIVEL;
      case TiposFiltro.dificuldade: return FILTRO_TEXTO_TIPO_DIFICULDADE;
      case TiposFiltro.ano: return FILTRO_TEXTO_TIPO_ANO; 
      //default: return null;
    }
  }
