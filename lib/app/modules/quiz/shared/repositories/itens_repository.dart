...
abstract class _ItensRepositoryBase with Store {
  ...
  _ItensRepositoryBase(this.firestoreRepository, this.assuntosRepository)
      : itensCollection =
            firestoreRepository.firestore.collection(DB_FIRESTORE_COLEC_ITENS) {
    carregarItens();
  }
  ...
  Future<List<Item>> carregarItens() async {
    ...
    if (resultado.docs.isEmpty) return List<Item>.empty();
    ...
    for (var snapshot in resultado.docs) {
      ...
      if (!_existeItem(map[DB_FIRESTORE_DOC_ITEM_ID])) {
        if (map.containsKey(DB_FIRESTORE_DOC_ITEM_REFERENCIA)) {
          ...
          _addInItens(
              Item.fromJson(map, assuntosRepository.assuntosCarregados));
        }
      }
    }
    return _itens;
  ...
  Future<Item?> _carregarItemReferenciado(
    ...
    if (_existeItem(ref.id))
      return _getItemById(ref.id);
    ...
    ///Criar e retornar o item se ele não estiver carregado.
      ...
      final _map = Map<String, dynamic>.from(
        dbItensSnapshot.docs
            .firstWhere((element) => element.id == ref.id)
            .data() as Map,
      );
