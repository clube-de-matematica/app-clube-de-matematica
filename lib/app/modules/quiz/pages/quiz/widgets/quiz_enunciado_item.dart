...
class EnunciadoItem extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return Observer(
      builder: (_) {
        if (controller.itensFiltrados.isEmpty) return const SizedBox();
        else {
          ...
          item.enunciado.forEach((texto) {
            ...
            if (controller.isImageNewLine(texto)) {
              if (blocosDeTexto.isNotEmpty) {
                ...
                lista.add(QuizTextFromBlocs(blocosDeTexto, textAlign));
                blocosDeTexto = [];
                textAlign = TextAlign.justify;
              }
              lista.add(
                ///Estrutura que conterá a imagem.
                Center(child: QuizComponenteImagem(item.imagensEnunciado[contador]))
              );
              contador++;
            }
                ...
                ///Estrutura que conterá a imagem.
                  WidgetSpan(
                      ...
                      child: Container(
                        ...
                        child: QuizComponenteImagem(item.imagensEnunciado[contador]),
                      )
                  )
              );
              contador++;
            ...
            ///`Katex` pesquisará em `texto` pelos marcadores de texto Latex "$" e "$$".
              ...
              KaTeX tex = KaTeX(laTeXCode: Text(texto));
