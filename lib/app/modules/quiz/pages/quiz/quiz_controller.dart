...
abstract class _QuizControllerBase with Store {
  ...
  String get textoContadorBarOpcoesItem => "${indice +1} de ${itensFiltrados.length}";
  ...
  Future<void> _carregarImagens () async {
    ...
    if (item.imagensEnunciado != null && item.imagensEnunciado.isNotEmpty)
        item.imagensEnunciado.forEach((imagem) {
          if (imagem.provider == null) 
              imagemItemRepository.setImagemProvider(imagem: imagem);
        });
    ...
    item.alternativas.forEach((alternativa) {
      if (alternativa.isTipoImagem) {
          if (alternativa.valorSeImagem.provider == null) 
              imagemItemRepository.setImagemProvider(imagem: alternativa.valorSeImagem);
      }
    });
  }
  ...
  Future<bool> onTapFiltrar() async {
    ...
    final retorno = await Modular.to.pushNamed<bool>(
        ROTA_PAGINA_FILTROS_TIPOS_PATH) ?? false;
